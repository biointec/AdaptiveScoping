...
for(t in c(10,20,30,40,50)){
# How many cycles?
n.cycles <-55 #----------------------------------------------------------------------------------------Cycles
  
scopingrate<-c(seq(from=1,to=0.1, length.out = t), rep(0.1,n.cycles-t))
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    hv.genome <- Genome$genome[[rep.iter]]
      ...
      #  burnIn <- 100
      #  thin <- 2
      #  bglr.model<- BGLR(y = TP.phenos.i,
      #                    ETA = list(list(X = TP.genos.i, model = "BRR",saveEffects = T)),
      #                    nIter = nIter,burnIn = burnIn,thin = thin, saveAt=paste0("Temp/rep",rep.iter,"/"),verbose=F)
                        
      #  t1=bglr.model$varE
      #  t2=predictions.out$solve.out$Ve
        
      #  t3=bglr.model$ETA[[1]]$varB
      #  t4=predictions.out$solve.out$Vu
        
      #  pred.info=list(rrblup= predictions.out, BGLR=bglr.model, verschil=list(Ve=(t2-t1),Vu=(t4-t3), lambda=(t3/t1-t4/t2)))
  ...
  filename <- file.path(save.dir, paste("AdaptiveScoping_Stepfunction_55BC_h02_", t ,"t_",n.iterations,"rep_",date, ".RData", sep = "") )                      
