...
for(t in c(10,20,30,40,50)){
  scopingrate<-c(seq(from=1,to=0.1, length.out = t), rep(0.1,70-t))
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    hv.genome <- Genome$genome[[rep.iter]]
      ...
      for (breeding.cycle in seq(n.cycles)) {
        ...
        #H_UC_bridging.R
        selection.pre<-OCS_H(candidate.marker.genos.i,
  ...
  filename <- file.path(save.dir, paste("t", t ,"_OCS_v2_",n.iterations,"rep_", change,"_",date, ".RData", sep = "") )                      
